...
- [🧮 数据结构 & 算法（程序 = 数据结构 + 算法）](#----------------------------)
    ...
    + [红黑树有什么更深刻理解?](#------------)
    + [什么是B树和B+树？](#---b--b---)
    + [如何快速判断一个数是奇数还是偶数，除了对2取余?](#--------------------2---)
- [☁️ 计算机网络](#--------)
    + [什么是内网什么是公网？](#-----------)
    + [什么是端口？端口的范围？](#------------)
    + [如何标识唯一标识一个连接？](#-------------)
    + [为什么有 TCP/IP 四层模型？层次结构设计的基本原则是什么？](#-----tcp-ip---------------------)
    + [对 TCP/IP 四层模型的理解？](#--tcp-ip---------)
    + [对 OSI 七层模型的理解？](#--osi---------)
    + [TCP & UDP 的区别](#tcp---udp----)
    + [什么是 http 协议，它在哪一层模型中？](#----http-------------)
    + [http 协议的特点有哪些？缺点有哪些？](#http----------------)
    + [http 协议不同版本的区别？](#http-----------)
    + [一次完整 HTTP 建立连接所经历的步骤？](#-----http------------)
    + [https 协议中的 s 是什么?](#https------s-----)
    + [https 协议有什么缺点？](#https---------)
    + [一次完整 HTTPS 建立连接所经历的步骤？](#-----https------------)
    + [https 密钥交换过程是什么？](#https-----------)
    + [三次握手怎么理解？](#---------)
    + [为什么握手不是两次或者四次？](#--------------)
    + [三次握手过程中可以携带数据么？](#---------------)
    + [Client & Server 同时发送 SYN 会怎么样?](#client---server------syn------)
    + [四次挥手怎么理解？](#---------)
    + [为什么挥手是四次而不是三次？如果是三次挥手会有什么问题？](#----------------------------)
    + [TOF 了解嘛？说说它的原理，说说它的优势？](#tof-------------------)
    + [TCP 报文中时间戳有什么用？](#tcp------------)
    + [现代浏览器在与服务器建立了一个 TCP 连接后是否会在一个 HTTP 请求完成后断开？什么情况下会断开？](#----------------tcp-----------http------------------)
    + [一个 TCP 连接可以对应几个 HTTP 请求？](#---tcp----------http----)
    + [一个 TCP 连接中 HTTP 请求发送可以一起发送么（比如一起发三个请求，再三个响应一起接收）？](#---tcp-----http----------------------------------)
    + [为什么有的时候刷新页面不需要重新建立 SSL 连接？](#-------------------ssl----)
    + [浏览器对同一 Host 建立 TCP 连接到数量有没有限制？](#-------host----tcp------------)
    + [到的 HTML 如果包含几十个图片标签，这些图片是以什么方式、什么顺序、建立了多少连接、使用什么协议被下载下来的呢？](#---html---------------------------------------------------)
    + [如何理解 TCP 的 keep-alive？](#-----tcp---keep-alive-)
    + [TCP 的超时重传时间是如何计算的？](#tcp---------------)
    + [能不能说一说 TCP 的流量控制？](#-------tcp-------)
    + [能不能说说 TCP 的拥塞控制？](#------tcp-------)
    + [怎么理解 rpc?](#-----rpc-)
    + [已经有 http 服务了为什么还有 rpc 服务？](#----http----------rpc----)
- [💾 数据库](#------)
    + [redis 和 memcache 有什么区别？使用场景](#redis---memcache-----------)
    + [redis 和 MySQL 有什么区别？](#redis---mysql-------)
    + [redis 的几种数据结构了解吗？](#redis------------)
    + [redis 为什么快？除了基于内存操作还有其他原因吗？](#redis----------------------)
    + [介绍一下 I/O 多路复用的流程？](#-----i-o---------)
    + [redis 的内存管理机制与gc？](#redis---------gc-)
    + [redis 如何保证高并发场景？](#redis-----------)
    + [redis 哨兵有了解吗？是干什么的有什么特性？](#redis-------------------)
    + [为什么哨兵至少是 3 个，2 个不可以吗？](#---------3---2-------)
    + [用 redis 来实现限制一个api或页面访问的频率，例如单ip或单用户一分钟之内只能访问多少次？](#--redis--------api------------ip-----------------)
    + [什么是索引？](#------)
    + [什么是联合索引？](#--------)
    + [什么是聚集索引、非聚集索引、覆盖索引?](#-------------------)
    + [给你ab,ac,abc字段，你是如何加索引的？](#--ab-ac-abc------------)
    + [索引可以创建多少列？](#----------)
    + [什么是视图？视图的使用场景有哪些？](#-----------------)
    + [视图的作用是什么？](#---------)
    + [事务的特性有哪些？](#---------)
    + [主键（primary key）和候选键（candidate key）有什么区别？](#---primary-key------candidate-key-------)
    + [主键和唯一键有什么区别?](#------------)
    + [数据库怎么优化查询效率？](#------------)
    + [了解数据库优化方案有哪些？](#-------------)
    + [写一条 SQL 最重要的是什么？](#----sql---------)
    + [千万级数据如何自动化并频繁写入 MySQL?](#----------------mysql-)
    + [如何将数据从一张表复制到另一张表？](#-----------------)
    + [如何在不复制数据的情况下复制表？](#----------------)
    ...
    + [MySQL锁有几种?](#mysql-----)
    + [死锁是怎么产生的?](#---------)
    + [char varchar text 有何区别?](#char-varchar-text------)
    + [区分 FLOAT 和 DOUBLE？](#---float---double-)
    + [了解join么，有几种，有何区别?](#--join-----------)
    + [NOW() 和 CURRENT_DATE() 有什么区别？](#now-----current-date---------)
    + [MySQL 表中允许多少个触发器？](#mysql------------)
    + [为何，以及如何分区、分表；](#-------------)
    + [关系型数据库和非关系型数据库？](#---------------)
...
- [💻 操作系统](#-------)
    ...
    + [什么是虚拟内存?](#--------)
- [🔱 正则专精](#-------)
    + [search和match的区别？](#search-match----)
    + [正则匹配 `<.*>` 和 `<.*?>` 分别对 `<h1>title</h1>` 进行正则匹配输出什么?](#--------------------------h1-title--h1--------------)
    + [正则匹配里的 * 和 + 有什么区别?](#-------------------)
    + [如何提高正则的可读性？](#-----------)
- [🕸️ 爬虫专精](#--------)
    + [网站如何监测爬虫使用了代理？](#--------------)
    + [在 App 逆向中常见的加密算法有哪些？](#--app---------------)
- [🐍 Python 语言特性](#---python-----)
    + [对面向对象的理解？](#---------)
    ...
    + [什么是元类？](#------)
    + [解释以下什么是闭包？](#----------)
    + [Python 的装饰器怎么理解？内部实现原理？](#python-----------------)
    + [什么是面向切面编程AOP?](#---------aop-)
    + [手写一个普通装饰器，一个带参装饰器？](#------------------)
    + [@staticmethod & @classmethod 有何区别？](#-staticmethod----classmethod------)
    + [解释一下实例方法中的 self](#-----------self)
    + [魔术方法的双下划线意义何在？](#--------------)
    + [hasattr() getattr() setattr() 函数使用详解？](#hasattr---getattr---setattr----------)
    + [将列表生成式中 [] 改成 () 之后数据结构是否改变?](#----------------------------)
    + [iterator & generator](#iterator---generator)
    + [new & init](#new---init)
    + [手写一个单例模式？说说理解](#-------------)
    + [enter & exit](#enter---exit)
    + [打开一个超大的 JSON 文件用 with 方法好吗？为什么不好？](#--------json-----with------------)
    + [简述read、readline、readlines的区别？](#--read-readline-readlines----)
    + [现在要处理一个大小为 10G 的文件，但是内存只有4G，应该如何实现？需要考虑的问题都有那些？](#-----------10g-----------4g--------------------)
    + [如何查看一个对象的内存？](#------------)
    + [怎么做更好？](#------)
    ...
    + [按 test_list 中元素的 age 由大到小排序](#--test-list------age-------)
    + [下面代码的输出结果将是什么？](#--------------)
    + [给定两个列表，怎么找出他们相同的元素和不同的元素?](#-------------------------)
    + [Dict 的底层使用什么实现的，查找的时间复杂度是多少？为什么？](#dict----------------------------)
    + [现有字典 `d = {'a':24,'g':52,'i':12,'k':33}` 请按value值进行排序?](#------d-----a--24--g--52--i--12--k--33-----value------)
    + [双向字典了解嘛，通过 value 找到 key？](#-----------value----key-)
    + [Python 已经有 list 结构为什么还要有 tuple?](#python-----list----------tuple-)
    + [python 是如何管理内存的？](#python----------)
    + [python 的垃圾回收机制是怎样的？](#python-------------)
    + [哪些操作会导致Python内存溢出，怎么处理？](#-------python----------)
    + [如何避免循环引用?](#---------)
    + [解释一下 GIL 锁](#-----gil--)
    + [什么是io密集型？什么是cpu密集型？](#---io-------cpu----)
    + [解释一下进程、线程、协程](#------------)
    + [线程是并发还是并行，进程是并发还是并行？](#--------------------)
    + [如何实现并行(parallel)和并发（concurrency)？](#-------parallel-----concurrency--)
    + [python 怎么实现多线程？](#python---------)
    + [multiprocessing 有了解嘛？和 asyncio 有什么区别？](#multiprocessing--------asyncio-------)
    + [python 的多线程不是真正的多线程，为什么还要用？](#python---------------------)
    + [如何最大性能执行 python](#---------python)
    + [为什么python代码执行缓慢？](#---python-------)
    + [如何提高执行速度？](#---------)
    + [python 的第三方库 Pipe 有用过吗？](#python-------pipe------)
    + [python 内置的弱引用 weakref 了解吗？](#python--------weakref-----)
    + [虚拟环境工具 poetry 了解吗？](#-------poetry-----)
    + [Python 实现定时任务有哪些方案？](#python-------------)
    + [阅读一下代码他们的输出结果是什么?](#-----------------)
- [🛠️ 应用框架](#--------)
    + [Django 和 Tornado 的关系、差别在哪里？](#django---tornado-----------)
    + [MVC / MTV；](#mvc---mtv-)
    + [缓存怎么用；](#------)
    + [中间件是干嘛的；](#--------)
    + [CSRF是什么，django是如何避免的；XSS呢；](#csrf----django-------xss--)
    + [如果你来设计login，简单的说一下思路；](#------login----------)
    + [session和cookie的联系与区别；session为什么说是安全的；](#session-cookie-------session---------)
    + [什么是WSGI,uwsgi,uWSGI?](#---wsgi-uwsgi-uwsgi-)
    + [为什么有了uWSGI为什么还需要nginx?](#-----uwsgi------nginx-)
    + [scrapy 框架工作流程](#scrapy-------)
- [🦳 白帽子](#------)
    + [解释 PoC & EXP 分别是什么？](#---poc---exp-------)
