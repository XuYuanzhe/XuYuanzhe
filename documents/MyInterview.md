...
- **15:00** 途游游戏 __现场面试__
  ...
  * <a href="#">朝阳区 暖山生活广场 B座3F</a>
    ...
    列举redis的五种数据类型，及应用场景，及底层数据结构？
      ...
      Set	String 类型的无序集合
      	点赞、签到、打卡、标签、抽奖
      List	存储有序的字符串(从左到右)，元素可以重复。可以充当队列和栈的角色。
      	消息队列。
      Zset
    如何使用redis实现分布式锁？
    redis的内存管理机制与gc？
...
是，现流行的AI人工智能技术大部分都是用Python语言编写的，这大大促进了的Python语言的发展。AI深度学习技术本身的特点决定了其不适合静态编译型语言，而Python语言被选作AI技术框架的基础语言，更多的是源于Python的动态特性及其开发效率高等性能优势。
...
-存疑- 错的，进程退出后操作系统会回收进程所占用的**全部**内存。这和GC没有关系。Python都退出了，GC当然一起退出了，退出以后由于循环引用、malloc申请的内存导致GC没清理干净的，会由**操作系统**统一回收, 确保资源不会泄漏。
